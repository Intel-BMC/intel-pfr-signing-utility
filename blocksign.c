...
HashAlg getHashAlgFromTcg(uint16_t nibble)
{
    ...
    switch (nibble)
    {
    case TPM_ALG_SHA384: ret = Sha384; break;
    case TPM_ALG_SHA512: ret = Sha512; break;
    }
    return ret;
}
const char *getHashStringFromTcg(uint16_t nibble)
{
    ...
    switch(nibble)
    {
    case TPM_ALG_SHA384: ret = HASH_SHA384_STR; break;
    case TPM_ALG_SHA512: ret = HASH_SHA512_STR; break;
    }
    return ret;
}


int doExternalSign(const unsigned char *dataRaw, const int rawLen, const unsigned char *hash,
    const int hashLen, const char* filename, unsigned char **r, unsigned char **s, int *externalLength, uint8_t verbose)
{
        ...
        if (ret)
        {
            ...
            if (ret)
            {
                ...
                if (ret)
                {
                    ...
                    if (fileReadChunk > 0)
                    {
                        ret = ExtractRs(readBuffer, fileReadChunk, r, s, externalLength);
                        if (!ret)
                        {
                            fprintf(stderr, "%sFailed to extract r/s from signature\n", getErr());
                        }
                    }
                    else
                    {
                        fprintf(stderr, "%sFailed to read anything from %s", getErr(), FILE_SIG_EXPECT);
                        ret = 0;
                    }
                }
                else
                {
                    fprintf(stderr, "%sFailed to open %s.", getErr(), FILE_SIG_EXPECT);
                }
            }
            else
            {
...
void hexDump(const unsigned char *buf, const int len, const char *pad)
{
    printf("%s%s0000: ", getNfo(), pad);
    int i;
    for(i = 0;i < len; ++i)
    {
        printf("%02X ", buf[i]);
...
int parseBlocks(struct ARGUMENTS *args)
{
    int i;
    int ret = 1;
    FILE *fp;
    ...
    if(ret)
    {
        ...
        {            
            ...
            if(ret)
            {
                readChunkSize = fread(&(b1.reserved1), 1, sizeof(b1.reserved1), fp);
                ...
                if(ret)
                {
                    readChunkSize = fread(&(b1.root_key), 1, sizeof(b1.root_key), fp);
                    ...
                    if(ret)
                    {
                        ...
                        {
                            readChunkSize = fread(&(b1.cs_key), 1, sizeof(b1.cs_key), fp);
                        ...
                        if(ret)
                        {
                            readChunkSize = fread(&(b1.block0_sig), 1, sizeof(b1.block0_sig), fp);
            ...
            else
            {
                ...
                if (args->blockpad != UINT32_MAX)
                {
                    if (args->blockpad > pos)
                    {
                        ...
                        {
                            fprintf(stderr, "%sConfig file block pad size (%d) is greater than file length (%ld)\n", getErr(), args->blockpad, ftell(fp));
                        }
                        else
                        {
                            pos = args->blockpad;
                    ...
                    {
                        fprintf(stderr, "%sConfig file block pad size (%d) is smaller than the actual block size (%ld)\n", getErr(), args->blockpad, pos);
                    }
                }
                ...
                for (i = 0; i < 32 && i < hashLen; ++i)
                {
                    ...
                    {
                        fprintf(stderr,"%s%s%s  *** Block 0 SHA256 does not match calculated value ***%s\n", getErr(), setAttribute(Bold), setAttribute(Red), setAttribute(Clear));
                        i = 255;
                    }
                }
                ...
                if (i == hashLen)
                {
                    printf("%s%s%s*** Block 0 SHA256 matches calculated value ***%s\n", getNfo(), setAttribute(Bold), setAttribute(Green), setAttribute(Clear));
                }
                printf("%s\n", getNfo());
                if (hash != NULL)
                {
                ...
                for (i = 0; i < 48 && i < hashLen; ++i)
                {
                    ...
                    {
                        fprintf(stderr, "%s%s%s  *** Block 0 SHA384 does not match calculated value ***%s\n", getErr(), setAttribute(Bold), setAttribute(Red), setAttribute(Clear));
                        i = 255;
                    }
                }
                if (i == hashLen)
                {
                    printf("%s%s%s*** Block 0 SHA384 matches calculated value ***%s\n", getNfo(), setAttribute(Bold), setAttribute(Green), setAttribute(Clear));
                }
                printf("%s\n", getNfo());
                if (hash != NULL)
                {
                ...
                hexDump(b0.reserved2, 32, "  ");
                printf("%s\n", getNfo());
                printf("%sBlock 1 Magic:           0x%08X\n", getNfo(), b1.magic);
                printf("%sBlock 1 Reserved(1):\n", getNfo());
                hexDump(b1.reserved1, 12, "  ");
                printf("%s\n", getNfo());
                printf("%s  Root Key Magic:        0x%08X\n", getNfo(), b1.root_key.magic);
                printf("%s  Root Key Curve Magic:  0x%08X\n", getNfo(), b1.root_key.curve_magic);
                printf("%s  Root Key Permissions:  %d\n", getNfo(), b1.root_key.permissions);
                printf("%s  Root Key Key ID:       %d\n", getNfo(), b1.root_key.keyid);
                printf("%s  Root Key Public (Qx):\n", getNfo());
                hexDump(b1.root_key.pubkey_x, 48, "    ");
                printf("%s\n", getNfo());
                printf("%s  Root Key Public (Qy):\n", getNfo());
                hexDump(b1.root_key.pubkey_y, 48, "    ");
                printf("%s\n", getNfo());
                printf("%s  Root Key Reserved1:\n", getNfo());
                hexDump(b1.root_key.reserved1, 20, "    ");
                printf("%s\n", getNfo());
                if(!(b0.pc_type & CANCELLATION_BIT))
                {
                    printf("%s  CS Key Magic:          0x%08X\n", getNfo(), b1.cs_key.magic);
                    printf("%s  CS Key Curve Magic:    0x%08X\n", getNfo(), b1.cs_key.curve_magic);
                    printf("%s  CS Key Permissions:    %d\n", getNfo(), b1.cs_key.permissions);
                    printf("%s  CS Key Key ID:    %d\n", getNfo(), b1.cs_key.keyid);
                    printf("%s  CS Key Public (Qx):\n", getNfo());
                    hexDump(b1.cs_key.pubkey_x, EC_POINT_MAX, "    ");
                    printf("%s\n", getNfo());
                    printf("%s  CS Key Public (Qy):\n", getNfo());
                    hexDump(b1.cs_key.pubkey_y, EC_POINT_MAX, "    ");
                    printf("%s\n", getNfo());
                    printf("%s  CS Key Reserved1:\n", getNfo());
                    hexDump(b1.cs_key.reserved1, 20, "    ");
                    printf("%s\n", getNfo());
                    printf("%s  CS KEY Sig Magic:      0x%08X\n", getNfo(), b1.cs_key.sig_magic);
                    printf("%s  CS KEY Sig (r):\n", getNfo());
                    hexDump(b1.cs_key.sig_r, EC_POINT_MAX, "    ");
                    printf("%s\n", getNfo());
                    printf("%s  CS KEY Sig (s):\n", getNfo());
                    hexDump(b1.cs_key.sig_s, EC_POINT_MAX, "    ");
                    printf("%s\n", getNfo());

                    // If params have been loaded, attempt to verify signatures
                    if (args->b1_args.root_key.pubkey != NULL && args->b1_args.cskey.hashalg != UINT16_MAX)
                    {
                        ...
                        ret = HashBuffer(buff, calcSize, getHashAlgFromTcg(args->b1_args.cskey.hashalg), &hash, &hashLen);
                        if (ret)
                        {
                            printf("%s  CS KEY contents %s hash dump:\n", getNfo(), getHashStringFromTcg(args->b1_args.cskey.hashalg));
                            hexDump(hash, hashLen, "    ");
                            printf("%s\n", getNfo());
                            // do size calculation if 32-48 are 0x00, then we assume 32
                            ...
                            ret = DerEncodeRs(b1.cs_key.sig_r, b1.cs_key.sig_s, pointLen, &sig, &sigLen);
                            if (ret)
                            {
                                ret = VerifyData(args->b1_args.root_key.pubkey, EcDsa, hash, getHashAlgFromTcg(args->b1_args.cskey.hashalg), sig, sigLen, &verified);
                                if (ret)
                                {
                                    if (verified)
                                    {
                                        printf("%s%s%s*** CS Key Signature Valid ***%s\n", getNfo(), setAttribute(Bold), setAttribute(Green), setAttribute(Clear));
                                    }
                                    else
                                    {
                                        fprintf(stderr, "%s%s%s*** CS Key Signature Invalid ***%s\n", getErr(), setAttribute(Bold), setAttribute(Red), setAttribute(Clear));
                                        ret = 0;
                                    }
                                    printf("%s\n", getNfo());
                                    verified = 0;
                                ...
                                {
                                    fprintf(stderr, "%sVerification function failed\n", getErr());
                                }
                            }
                            else
                            {
                                fprintf(stderr, "%s  DER encoding of signature failed", getErr());
                                ret = 0;
                ...
                {
                    fprintf(stderr,"%s  Skipping CS Key parsing. PC Type is Key Cancellation\n", getWrn());
                    printf("%s\n", getNfo());
                }
                printf("%s  B0 Magic:              0x%08X\n", getNfo(), b1.block0_sig.magic);
                printf("%s  B0 Sig Magic:          0x%08X\n", getNfo(), b1.block0_sig.sig_magic);
                printf("%s  B0 Sig Sig (r):\n", getNfo());
                hexDump(b1.block0_sig.sig_r, 48, "    ");
                printf("%s\n", getNfo());
                printf("%s  B0 Sig Sig (s):\n", getNfo());
                hexDump(b1.block0_sig.sig_s, 48, "    ");
                printf("%s\n", getNfo());
                // If params have been loaded, attempt to verify signatures
                if (args->b1_args.cskey.pubkey != NULL && args->b1_args.b0sig.hashalg != UINT16_MAX)
                {
                    printf("%s  B0 Sig Signature Verification\n", getNfo());
                    calcSize = sizeof(b0);
                    buff = &b0;
                    ret = HashBuffer(buff, calcSize, getHashAlgFromTcg(args->b1_args.b0sig.hashalg), &hash, &hashLen);
                    if (ret)
                    {
                        printf("%s  B0 contents %s hash dump:\n", getNfo(), getHashStringFromTcg(args->b1_args.b0sig.hashalg));
                        hexDump(hash, hashLen, "    ");
                        printf("%s\n", getNfo());
                        // do size calculation if 32-48 are 0x00, then we assume 32
                        int pointLen = EC_POINT_256;
                        ...
                        ret = DerEncodeRs(b1.block0_sig.sig_r, b1.block0_sig.sig_s, pointLen, &sig, &sigLen);
                        if (ret)
                        {
                            ret = VerifyData(args->b1_args.cskey.pubkey, EcDsa, hash, getHashAlgFromTcg(args->b1_args.b0sig.hashalg), sig, sigLen, &verified);
                            if (ret)
                            {
                                if (verified)
                                {
                                    printf("%s%s%s*** Block0 Signature Valid ***%s\n", getNfo(), setAttribute(Bold), setAttribute(Green), setAttribute(Clear));
                                }
                                else
                                {
                                    fprintf(stderr, "%s%s%s*** Block0 Signature Invalid ***%s\n", getErr(), setAttribute(Bold), setAttribute(Red), setAttribute(Clear));
                                    ret = 0;
                                }
                                verified = 0;
                            }
...
int generateBlocks(struct ARGUMENTS *args)
{
    struct BLOCK_0 b0;
    struct BLOCK_1 b1;
    // zero out b0 and b1
    ...
    // if we need to align, then read/write input with alignment padding.
    if (args->align != 0 && args->align != UINT32_MAX && ret != 0)
    {
        if (args->verbose)
        {
            printf("%sGenerating intermediate file with proper alignment padding.\n", getNfo());
        }
        ...
        if (ret)
        {
            ...
            if (ret)
            {
                ...
                {
                    writeChunkSize = fwrite(chunk, sizeof(unsigned char), FILE_CHUNK_SIZE, ifp);
                ...
                writeChunkSize = fwrite(chunk, sizeof(unsigned char), readChunkSize, ifp);
                padamnt = (args->align) - ((readChunkSize + actualBlockPad) % (args->align));    
                ...
                {
                    writeChunkSize = fwrite(chunk, sizeof(unsigned char), 1, ifp);
                    if (writeChunkSize != 1)
                    {
                        ret = 0;
                    }
    ...
    if (ret)
    {
        ...
        if (ret)
        {
            ...
            ret = ExtractQxQyFromPubkey(args->b1_args.root_key.pubkey, &qx, &qy, &size);
            if (ret)
            {
                if (size <= 48)
                {
        ...
        if (ret && !(b0.pc_type & CANCELLATION_BIT))
        {
            ...
            ret = ExtractQxQyFromPubkey(args->b1_args.cskey.pubkey, &qx, &qy, &size);
            if (ret)
            {
            ...
            if (ret)
            {
                ...
                if (args->verbose)
                {
                    printf("%s    Calculating data buffer and hashing with %s.\n", getNfo(), getHashStringFromTcg(args->b1_args.cskey.hashalg));
                }
                // calculate the size of the buffer that we should hash (this should prevent accessing bad mem)
                calcSize = sizeof(b1.cs_key.curve_magic) +
                    sizeof(b1.cs_key.permissions) +
                    sizeof(b1.cs_key.keyid) +
                    sizeof(b1.cs_key.pubkey_x) +
                    ...
                    sizeof(b1.cs_key.reserved1);
                buff = &(b1.cs_key.curve_magic);
                ret = HashBuffer(buff, calcSize, getHashAlgFromTcg(args->b1_args.cskey.hashalg), &hashBuffer, &size);
                if (args->verbose)
                {
                    printf("%s    Hash Dump:\n", getNfo());
                    hexDump(hashBuffer, size, "      ");
                }
                if (ret)
                {
                    if (args->b1_args.cskey.sign_key != NULL)
                    {
                        ...
                        ret = SignData(args->b1_args.cskey.sign_key, EcDsa, hashBuffer, getHashAlgFromTcg(args->b1_args.cskey.hashalg), &sig, &sigLen);
                        if (ret)
                        {
                            ...
                            if (ret)
                            {
                                ...
                                if (size > EC_POINT_MAX)
                                {
                                    fprintf(stderr, "%sSig(r/s) points are too large. Current implementation supports up to 384 bit prime field (i.e. secp384r1).\n", getErr());
                                    ret = 0;
                                }
                                for (i = 0; i < size; ++i)
                                {
                    ...
                    {
                        ret = doExternalSign(buff, calcSize, hashBuffer, size, args->b1_args.cskey.script_file, &r, &s, &externalSize, args->verbose);
                        if (ret)
                        {
                            if (args->verbose)
                            {
                                printf("%s    Sig(r):\n", getNfo());
                                hexDump(r, externalSize, "      ");
                                printf("%s    Sig(s):\n", getNfo());
                                hexDump(s, externalSize, "      ");
                            }
        ...
        if (ret)
        {
            ...
            ret = HashBuffer(buff, calcSize, getHashAlgFromTcg(args->b1_args.b0sig.hashalg), &hashBuffer, &size);                
            if(args->verbose)
            {
                printf("%s    Hash Dump:\n", getNfo());
                hexDump(hashBuffer, size, "      ");
            ...
            if (ret)
            {
                if (args->b1_args.b0sig.sign_key != NULL)
                {
                    if (args->verbose)
                    {
                        printf("%s    Signing hash with key %s\n", getNfo(), args->b1_args.b0sig.sign_key);
                    }
                    ret = SignData(args->b1_args.b0sig.sign_key, EcDsa, hashBuffer, getHashAlgFromTcg(args->b1_args.b0sig.hashalg), &sig, &sigLen);
                    if (ret)
                    {
                        ret = ExtractRs(sig, sigLen, &r, &s, &size);
                        if (ret)
                        {
                            ...
                            if (size > EC_POINT_MAX)
                            {
                                // curve points too big, unsupported. 384 or smaller
                                ret = 0;
                            }
                            for (i = 0; i < size; ++i)
                            {
                ...
                {
                    ret = doExternalSign(buff, calcSize, hashBuffer, size, args->b1_args.b0sig.script_file, &r, &s, &externalSize, args->verbose);
                    if (ret)
                    {
                        if (args->verbose)
                        {
        ...
        if (ret)
        {
            ...
            if (ret)
            {
                if (args->blockpad != UINT32_MAX)
                {
                    if (args->verbose)
                    {
                        printf("%s  Calculating amount of bytes to pad blocks.\n", getNfo());
                    }
                    // recalculate padding now that we know the size of the blocks
                ...
                writeChunkSize = fwrite(&b0, sizeof(unsigned char), sizeof(b0), ifp);
                if (args->verbose)
                {
                    printf("%s  Writing Block1.\n", getNfo());
                }
                writeChunkSize = fwrite(&(b1.magic), sizeof(unsigned char), sizeof(b1.magic), ifp);
                writeChunkSize = fwrite(&(b1.reserved1), sizeof(unsigned char), sizeof(b1.reserved1), ifp);
                writeChunkSize = fwrite(&(b1.root_key), sizeof(unsigned char), sizeof(b1.root_key), ifp);
                if (!(b0.pc_type & CANCELLATION_BIT))
                {
                    writeChunkSize = fwrite(&(b1.cs_key), sizeof(unsigned char), sizeof(b1.cs_key), ifp);
                }
                else if (args->verbose)
                {
                    printf("%s  PC Type is key cancellation, skipping Block 1 CSK structure.\n", getNfo());
                }
                writeChunkSize = fwrite(&(b1.block0_sig), sizeof(unsigned char), sizeof(b1.block0_sig), ifp);
                chunk[0] = PAD_BLOCK;
                if (args->verbose && (int)actualBlockPad > 0)
                {
                    printf("%s  Padding %d bytes (0x00) to the end of Block1.\n", getNfo(), actualBlockPad);
                }
